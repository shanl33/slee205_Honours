...
# Numerical method: Heirarchical clustering algorithm using Euclidean distances and average linkage method
...
# Scattermatrix (highlight by cluster) used to identify following:
...
# Cluster 2 is the smaller main cluster (with relatively lower success, esp at L2. Consists of schools from the lower tail for each level).
# A lot of overlap between Cluster 1 and 2.
...
# Findings ----------------------------------------------------------------
# Two clusters:
# Apart from one school, all of the remaining schools in the smaller, lower cluster are at decile 6 or below.
# And generally consists of the lower tail of the distributions of achievement at each level, but there is a lot of overlap between the two clusters.
# 4 outliers:
# The 4 schools that were outliers were from decile 5 or below. None had UE at 100% but had other levels at 100%.
# They were identified as unusual due to inconsistencies in performance across the levels (100% at one or more levels, but unexpectedly low performance at other levels)
# Overall:
# There appears to be a bi-modal distribution at L3 (and very slight emergence at L2)
# Visualisations:
# Scatterplot matrix: being able to color by cluster and view 1D ASH (histograms)
# Pcp: identify (outlier) cases and view cluster patterns across vars. 
# BUT Very difficult to brush/identify cases when many nearby.
# 2D tour: was difficult deciding when to pause the tour and hence which projection showed the 'best' separation.
# This data is quite possibly just difficult to separate visually? (Dendrogram did not indicate clear separation)
# The projection was successfully transferred to R but not sure how the linear combination coefficients are stored and accessed in ggobi or the R output.
...
str(tour) # Are the linear combinations for the projection saved anywhere?
ggplot(tour)
