...
ui <- fluidPage(
  tabsetPanel(
    ...
    tabPanel("PCP", inputPanel(
      selectInput("scaling", "Select scale for axes", choices=c("Standardise", "Univariate_minmax", "Global_minmax"), selected="Standardise"),
      div(style="width: 600px;",
          sliderInput("cohort3", label="Minimum size of Year 11, 12, 13 cohorts", min=0, max=100, step=10, value=0, animate=T))
      ),
      mainPanel(plotlyOutput("pcp"))
    )
)
)
...
server <- function(input, output) {
  ...
  output$pcp <- renderPlotly({
    nzqa <- subset(nzqa.sch, Min.Cohort>=input$cohort3, 
                   select=c(School, L1, L2, L3, UE, Decile, Min.Cohort))
    if (input$scaling == "Standardise") {
      nzqa.scale <- cbind(nzqa[-(2:5)], scale(nzqa[2:5]))
    } else if (input$scaling == "Univariate_minmax") {
      nzqa.scale <- cbind(nzqa[-(2:5)], rescale(nzqa[2:5]))
    } else {
      nzqa.scale <- nzqa
    }
