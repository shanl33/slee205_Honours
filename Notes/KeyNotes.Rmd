...
* Is `cranvas` worth investigating.
    + See <http://cranvas.org/>  
    + *aims to be the next generation of GGobi*
    + BUT not on CRAN and last update seems to be 2013
    + Demos shows brush linking, zooming, alpha blending.
    + `qparallel()` specifically for parallel coord plots
    + Installation of `Qt` required
...
* Tableau <https://public.tableau.com/s/>
    - See if there are any interaction techniques that have not been demonstrated using other tools (esp ggobi)
