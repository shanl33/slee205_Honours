...
* "Export" the linear combination coordinates from tours using DescribeDisplay. (This is most likely in the dd_object, I just haven't figured out how to access it).
...
### To do:
* Check for equal variance between decile groups and see if can use LDA or another parametric method (rather than algorithmic).
...
1. Sphere data (transform to principal components)
    + Recommended since easier to bring out interesting projections, especially when using projection pursuit indices.
