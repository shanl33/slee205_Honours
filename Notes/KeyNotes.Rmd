...
## Questions
* Is it sensible for `plotly` plots to render faster than `ggplot2`?
    + `plotly` object keeps up better with animation at a faster speed (300 millisecond per animation step)
* Apart from using `shiny`, how else can you be in a "reactive environment" (need this to explicitly retrieve SharedData details)
eg. Use print(sd$data(withSelection=TRUE))
* Using `plotly` alone (without `shiny`) has the advantage of producing a "standalone html". Why is this an advantage? Easier to share, no need to host on an external server (?) 
...
* Tour function (using `tourr` package)
    + Investigate and visually display the effect of the starting projection matrix on the random tour path, by creating the plot described in the above point, and creating a scatterplot matrix of pairs of PC's as possible starting matrices. Click on scatterplot pair to select as starting matrix and initiates a few random tour paths with the traces of their paths displayed for comparison.
        - An equivalent to `pairs()` plot in `GGally` or `plotly`? Or just have a static pairs plot and checkboxes on the side?
...
* "Export" the linear combination coordinates from tours using DescribeDisplay. 
    + Allow for comparison with numerical methods.
...
[^2]: Venables, Ripley (1999) MASS book
