...
## Questions
* Do I have to cite a specific source when making "common statements"?
    + eg. Order of vars for a parallel coord point is impt?
    + Choice of file type for image (raster vs scalable)
* `Acinonyx` R package is a possible successor for `iplots`? (Unwin)
* `Trelliscope` talks back to R? (Its js library and R package library are the same entity)
* `rbokeh` similar to `plotly`? 
    + `rbokeh` requires more code to display hover tips
    + `rbokeh` + trelliscope has explicit examples, not so with `plotly` (currently I cannot get working)
...
* Trelliscope is like trellis plots with the added interactivity of being able to filter and sort panels of multiple trellis plots based on "cognostic" summaries of the data.
    + "cognostic" = diagnostics guided by computers (term proposed by Tukey)
    + Cognostics in trelliscope: Statistical summaries automatically calculated for the subsets that are created by interactive filtering.
    + Defualt cognostics automatically calculated by trelliscope:
...
* Trelliscope produces *small multiple plots which can be navigated with respect to the summaries*
...
* `trelliscopejs` is an `htmlwidget` interface to the JavaScript library by the same name (`trelliscopejs-lib` - allows for the interactive viewing). Uncommon for both JavaScript and R libraries to be in the same entity - what are the advantages? (talks back to R??)

### Trelliscope and `rbokeh`: `ly_points()` has `hover=` argument.
    * See <https://hafen.github.io/trelliscopejs/#getting-started>
    * Can use `plotly` insteand of `rbokeh`?
...
* Trelliscope graphics can be shared as standalone apps or embedded in RMarkdown notebooks.
...
### Disadvantages
* Underdevelopment
* Specific to *small multiples* interaction (filtering and sorting of trellis plots). 
