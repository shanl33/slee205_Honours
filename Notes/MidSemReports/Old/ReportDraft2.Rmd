...
# To do's:
+ Screenshot of shiny app and put it on shiny.
+ Add to intro.
+ Check all points are demonstrated in part 2.

A literature review was undertaken to identify commonly used interactive techniques and hence the techniques that should be prioritised when teaching interactive data visualisation. The survey of current software tools was then guided by their ability to enable/deliver these techniques.
...
(add more details on method for surveying)
...
## Commonly used interactive techniques
...
## Capabilites of current software
...
```{r table1, results='asis', fig.cap="Interactive techniques for Shiny, Plotly and Crosstalk."}
tabl <- "
...
```{r table2, results='asis', fig.cap="Linked brushing techniques."}
tabl2 <- "
...
ncea_PCA <- function(df, scale.factor=20, nudge=-0.2, gp="Decile", sd.group=NULL) {
  # Perform PCA
  ...
  PCAplot <- ggplot(loadings) + 
    ...
    geom_point(data=sdPCA, aes(x=PC1, y=PC2, colour=Group, label1=School)) +
...
# Static plot (with no legend)
akl.PCA <- ncea_PCA(akl, sd.group="AKLncea") + 
  labs(title="Achievement of Auckland schools in 2016")

akl.PCA + 
  theme(legend.position = "right", legend.key=element_rect(fill="white")) +
  guides(colour=guide_legend("Decile", reverse=TRUE))
```

The colour scheme in \@ref(fig:pcaAkl) highlights that apart from two schools, decile nine and ten schools performed similarly. Meanwhile schools from the remaining deciles were more spread out from each other in the PCA plot. It is difficult not to be affected by the colour scheme when identifying patterns in Figure \@ref(fig:pcaAkl). Unwin(???) emphasised the influence of colour choice can have on conclusions drawn during graphical data analysis. 

Finding a colour scheme that sufficiently distinguishes between the 11 levels of the decile factor and avoids "dominating colours", would be challenging, if not impossible. Furthermore since there is a sequential order to the decile levels, trends such as those identified for higher decile schools would be hard to see in Figure \@ref(fig:pcaAkl), if a purely qualitative colour scheme had been applied.

Linked brushing in Figure ?? enables subset selection by decile level. 

```{r}
# Static PCA already defined (akl.PCA)
...
subplot(link.PCA, link.decile, widths=c(0.7, 0.3), margin=0, titleX=T, titleY=T) %>%
  layout(dragmode="select") %>%
  highlight(on="plotly_select", off="plotly_deselect", color="blue", persistent=T)
```


Confirms then grouping of decile 9 and 10?

```{r}
include_graphics("files/akl9_10.jpg", auto_pdf=F)
```


The principal components plots provided a view of the multivariate distribution of the Auckland schools data set that was less easily affected by overplotting than a static PCP. Hence refinements on observations were possible. From the parallel coordinate plots we observed that "higher" decile schools performed more consistently with each other and dominating across the four qualification levels, but it was difficult to quantify the decile "cut off" for such schools. The PCA suggests these schools are the decile nine and ten schools in Auckland, with the exception of two schools. Again interactive tooltips enable instant identification of these two unusual observations, in Figure \@ref(fig:pcaInt).
...
ncea_factor <- function(df, gp="Decile", sd.group=NULL) {
