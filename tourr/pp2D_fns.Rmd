...
pp2D_shiny <- function (dataset, index="cmass", group=NULL) {
  ...
  # Initial projection as scatterplot with first two measures
  #d <- dim(t) # dimensions: p (# of Xs), dimensions (2), # of bases
  #t0 <- matrix(c(1, rep(0, d[1]), 1, rep(0, (d[1]-2))), ncol = 1)
  #t1 <- c(t0, t)
  #d[3] <- d[3]+1
  #dim(t1) <- d
...
```{r xtalk, echo=FALSE}
pp2D_xtalk <- function(dataset, index="cmass", group=NULL) {
  # Subset real-valued vars (type="double") for touring
  num_cols <- sapply(dataset, typeof)=="double"
  Xdataset <- dataset[, num_cols]
  # Save tour
  ...
  # matrix(3rd_comp, ncol=2) converts it into a projection matrix, A.
  t_tour <- apply(tinterp, 3, FUN = cmass_tour)
  proj <- data.frame(Name=rep(rownames(dataset), dim(tinterp)[3]), step=rep(1:dim(tinterp)[3],each=dim(Xdataset)[1]), x=c(), y=c())
  for(i in 1:length(t_tour)) {
    pp_index$index[i] <- unlist(t_tour[[i]][2])
    proj$x <- c(proj$x, unlist(t_tour[[i]][[1]][,1]))
    proj$y <- c(proj$x, unlist(t2_tour[[i]][[1]][,2]))
  }
  # tour plot
  tour <- proj %>%
  ...
  axis_p <- ggplot(basis, aes(x=V1, y=V2)) +
        ...
        scale_color_gradient(low = "#56B1F7", high = "#132B43")
  axis_p <- ggplotly(tooltip = "none")
  tour <- subplot(tour_p, axes_p, nrows = 1, shareY = T, margin = 0) %>%
